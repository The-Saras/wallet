            ...
            <div className="border border-slate-700 p-8 mx-10">
              ...
              <div className="grid grid-cols-4 gap-4">
                {mnemonicWords.map((word, index) => (
                  <div key={index} className="bg-slate-800 text-white p-4 rounded-md text-center border border-slate-700 hover:bg-slate-700">
                    {word}
                  </div>
                ))}
              </div>
              <div className="flex items-center py-4 px-4">
                <ClipboardIcon className="w-6 h-6 text-blue-500 cursor-pointer hover:text-blue-600" onClick={copyToClipboard} />
                <span className="ml-2 text-lg text-slate-200 cursor-pointer hover:underline font-sans text-slate-400" onClick={copyToClipboard}>
                  Copy to Clipboard
                </span>
              </div>
            </div>
